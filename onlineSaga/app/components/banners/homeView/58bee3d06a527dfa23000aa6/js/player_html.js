	...
	function preloadAssets (scene, includeVideo, callback) {
		...
		function loadWeatherFactory(objectID) {
			return function loadWeatherForecast(cb) {
				...
				if (cacheStorage) {
					weatherForecasts[objectID] = JSON.parse(cacheStorage);
					cb();
				}
				global.yahooForecast(sceneAssets[objectID], function(err, data) {
					if (!err) {
						weatherForecasts[objectID] = data;
		...
		*/
		...
		this.weather = function renderWeatherWidget(object) {
			if (weatherForecasts[object.id]) {
				...
				forecastIcon.innerHTML = String.fromCharCode(weatherForecasts[object.id].mainForecast.icon);
				...
				maxTemp.innerHTML = weatherForecasts[object.id].mainForecast.max + '&deg;';
				...
				minTemp.innerHTML = weatherForecasts[object.id].mainForecast.min + '&deg;';
				...
				if (object.forecasts > 0) {
					...
					if (object.layout === 'h') {
						...
						for (i = 0; i < object.forecasts; i++) {
							// Safeguard
							if (!weatherForecasts[object.id].extraForecasts[i]) {
								break;
							}
							...
							iconText.innerHTML = String.fromCharCode(weatherForecasts[object.id].extraForecasts[i].icon);
							...
							fMax.style.bottom = '0';
							fMax.style.left = '50%';
							fMax.innerHTML = weatherForecasts[object.id].extraForecasts[i].max + '&deg;';
							...
							fMin.style.bottom = '0';
							fMin.style.right = '50%';
							fMin.innerHTML = weatherForecasts[object.id].extraForecasts[i].min + '&deg;';
